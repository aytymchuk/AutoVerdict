...
- **Mapping lists to domain models**: When mapping a Mongo query result (`List<TDocument>` from `ToListAsync`) to domain records, use `list.ConvertAll(Mapper.ToDomain)` instead of `list.Select(Mapper.ToDomain).ToList()` — it avoids the iterator allocation and is the established convention (see `ResearchRepository.cs`). Only applies when the source is statically typed `List<T>`; use `Select(...).ToList()` when the source is `IEnumerable<T>` or `IReadOnlyList<T>` (which have no `ConvertAll`).
